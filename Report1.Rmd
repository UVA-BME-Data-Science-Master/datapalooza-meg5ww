...
All data sets used in this study were obtained from GEO. Two separate data sets comparing the effects of doxorubicin on cardiomyocytes were studied. The first data set compared human embryonic stem cell derived cardiomyocytes that were treated with 0, 1, or 2.5 uM of doxorubicin for 16 hours (4). The second data set studied obtained human-induced pluripotent stem cell derived cardiomyocytes from six female breast cancer patients that were treated with doxorubicin. Three of these patients experienced cardiotoxicity as a result of doxorubicin treatment, whereas three did not. These stem cells were then treated with either 0 or 1 uM of doxorubicin for 24 hours (1). Both data sets were processed via the platform Illumina HiSeq 2000 for high throughput sequencing. 
...
# Preliminary Results
Salmon was used to quantify the RNA sequencing data from the GSE106297 dataset. Raw data for the 0 and 1 uM doxorubicin treatment conditions were obtained from GEO/SRA, and converted to FASTQ format using the SRA toolkit (due to the large size of the runs, the analysis was performed using the Rivanna High-Performance Computing Cluster):
...
In the above code, SRR6231704 represents one of the SRA runs downloaded and converted to FASTQ format. SRA runs SRR6231704-SRR6231711 were used in this analysis, as these are the runs corresponding to the 0 uM doxorubicin (SRR6231704-SRR6231707) and 1 uM doxorubicin (SRR6231708-SRR6231711) treatment conditions. Since the SRA files contained paired-end sequencing data, `--split-files` was needed to produce two FASTQ files per run, one for R1 and one for R2. 
...
# assign experimental variables - 0 or 1 uM doxorubicin treatment
...
![Figure 1. Top differentially expressed genes between 1 uM and 0 uM doxorubicin treatment for the GSE106297 dataset (gene IDs have not yet been converted to gene names).](de_output.png)
